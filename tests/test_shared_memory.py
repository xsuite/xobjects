...
def test_shared_memory():
    for test_context in xo.context.get_test_contexts():
        ...
        if isinstance(test_context, xo.ContextCupy):
            ...
            test_context.default_shared_mem_size_bytes=32  # 4 * 8 bytes = memory for 4 doubles
            print(f"[test_shared_memory.py] default_block_size: {test_context.default_block_size}, default_shared_mem_size_bytes: {test_context.default_shared_mem_size_bytes}")
        else:
            print("[test_shared_memory.py] skipping test for cpu and pyopencl context")
            continue            
    
        _test_shared_memory_kernel = xo.Kernel(
             c_name="test_shared_memory",
             args=[xo.Arg(xo.Float64, const=True, pointer=True, name='input_arr'),
                   xo.Arg(xo.Float64, pointer=True, name='result'),
                   xo.Arg(xo.Float64, const=True, name='n')],
                   n_threads="n",
        )
    
        _test_shared_memory_kernels = {'test_shared_memory': _test_shared_memory_kernel,
                                }
        ...
        n=4
        input_arr = test_context.nplike_lib.array([1.3, -5.6, 0.01, 90], dtype=np.float64)
        ...
        telem._context.kernels.test_shared_memory(input_arr=input_arr, result=result, n=np.int64(n))
